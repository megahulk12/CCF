	...
	if(isset($id)) {
		...
		if(mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_assoc($result)) {
				...
				$array = array("headID"=>$headID, "picturepath"=>$picturepath, "name"=>$name, "description"=>$description, "startday"=>$startday, "endday"=>$endday, "weekly"=>$weekly, "starttime"=>$starttime, "endtime"=>$endtime, "venue"=>$venue, "schedstatus"=>$schedstatus);
		...
		if($confirmUpload) {
			...
			$sql_propose_event = "UPDATE eventdetails_tbl SET eventPicturePath = '$eventpicturepath', eventName = '$eventname', eventDescription = '$eventdesc', eventStartDay = '$eventstartdate', eventStartTime = '$starttime', eventEndTime = '$endtime', eventVenue = '$venue', eventSchedStatus = $eventschedstatus WHERE eventID = ".$id;
			}
			else if($eventschedstatus == 1) {
			$sql_propose_event = "UPDATE eventdetails_tbl SET eventPicturePath = '$eventpicturepath', eventName = '$eventname', eventDescription = '$eventdesc', eventStartDay = '$eventstartdate', eventEndDay = '$eventenddate', eventStartTime = '$starttime', eventEndTime = '$endtime', eventVenue = '$venue', eventSchedStatus = $eventschedstatus WHERE eventID = ".$id;
			}
			else if($eventschedstatus == 2) {
			$sql_propose_event = "UPDATE eventdetails_tbl SET eventPicturePath = '$eventpicturepath', eventName = '$eventname', eventDescription = '$eventdesc', eventStartDay = '$eventstartdate', eventEndDay = '$eventenddate', eventWeekly = '$weekly', eventStartTime = '$starttime', eventEndTime = '$endtime', eventVenue = '$venue', eventSchedStatus = $eventschedstatus WHERE eventID = ".$id;
			}
