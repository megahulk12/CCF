<?php
	if(isset($_GET['id'])) {
		...
		$sql_ministry_details = "SELECT ministyName, ministryDescription, schedDate, schedDay, schedStartTime, schedEndTime, schedPlace, remarks, schedStatus FROM ministrydetails_tbl LEFT OUTER JOIN scheduledmeeting_tbl ON ministrydetails_tbl.schedID = scheduledmeeting_tbl.schedID WHERE ministryID = $id";
		...
		if(mysqli_num_rows($result) > 0) {
			...
			while($row = mysqli_fetch_assoc($result)) {
				...
				$startday = date("F j, Y", strtotime($row["ministryStartDay"]));
				$endday = date("F j, Y", strtotime($row["ministryEndDay"]));
				$weekly = $row["ministryWeekly"];
				$starttime = $row["ministryStartTime"];
				$endtime = $row["ministryEndTime"];
				$venue = $row["ministryVenue"];
				$remarks = $row["remarks"];
				$schedstatus = $row["ministrySchedStatus"];
